...
export default function HeroSection() {
  ...
  // Handle canvas rendering and GSAP scroll logic
  useEffect(() => {
    ...
    const trigger = ScrollTrigger.create({
      ...
      start: "top top",
      end: "+=250%", // Pins the section for 250vh of scrolling
      pin: true,
      scrub: 0.1, // Smooth scrubbing
      onUpdate: (self) => {
        const frameIndex = Math.max(1, Math.min(TOTAL_FRAMES, Math.ceil(self.progress * TOTAL_FRAMES)));
  ...
  const getLayerOpacity = (start: number, end: number) => {
    ...
    // If this is the final layer, do not fade out so it stays visible until the next section appears
    if (end === TOTAL_FRAMES) {
  ...
  return (
    <div id="home" ref={containerRef} className="relative w-full bg-primary h-[100dvh] overflow-hidden">
      {/* Loading Overlay */}
      {!isLoaded && (
        <div className="absolute inset-0 bg-primary z-50 flex flex-col justify-center items-center">
      ...
      <div
        className="absolute inset-0 flex flex-col justify-center items-center text-center px-6 transition-all duration-300 pointer-events-none z-20"
        style={{
      ...
      {activeFrame < 130 && (
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center z-30 pointer-events-none animate-bounce-slow">
